                ...
                <div className="pt-2 md:pt-0 flex flex-col ">
                    ...
                    <p className="grow self-auto md:hidden">
                        {(strInstructions.split(' ').length >= MOBILE_MAX_WORDS) ? 
                            strInstructions.split(' ').slice(0, MOBILE_MAX_WORDS).join(' ') + ' ...' :
                            strInstructions}
                    </p>
                    <p className="grow self-auto hidden md:block">
                        {(strInstructions.split(' ').length >= DESKTOP_MAX_WORDS) ? 
                            strInstructions.split(' ').slice(0, DESKTOP_MAX_WORDS).join(' ') + ' ...' :
                            strInstructions}
                    </p>
                    <button 
                        className="p-2 mt-2 border-2 border-orange-900 rounded-lg 
                                bg-orange-500 hover:bg-white hover:text-2xl text-orange-900
                                lg:text-xl"
