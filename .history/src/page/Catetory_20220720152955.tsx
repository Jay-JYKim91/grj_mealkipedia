...
const Category: React.FC = () => {
    ...
    const filteredRecipes = useQuery(
        ...
        () => apiMeals.getFilteredRecipes(selectedCategory),
        {
            onSuccess: (res) => {
                ...
                setData(res.slice(0, itemCount + 1));
            },
        }
    );
    ...
    //     console.log(filteredRecipes.data.length);
    //     // if (filteredRecipes.data.length > itemCount) {
    //     setData(filteredRecipes.data.slice(0, itemCount));
    //     // }
    ...
    useEffect(() => {
        if (filteredRecipes.data && filteredRecipes.data.length > itemCount) {
            setData(filteredRecipes.data.slice(0, itemCount + 1));
        }
    }, [itemCount]);
